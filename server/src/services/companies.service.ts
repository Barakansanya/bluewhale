...
export class CompaniesService {
  ...
  async getById(identifier: string) {
    const ticker = identifier.toUpperCase().replace('.JO','');
    const company = await prisma.company.findFirst({
      ...
      include: {
        ...
        financials: { orderBy: { fiscalYear: 'desc' }, take: 5 },
      },
    });

    if (!company) throw new Error('Company not found');

    return {...company, metrics: company.metrics[0] || null };
  }
  ...
  async getCompanyByTicker(ticker: string) {
    const company = await prisma.company.findUnique({
      ...
      include: {
        metrics: { orderBy: { asOfDate: 'desc' }, take: 1 },
        historicalPrices: { orderBy: { date: 'desc' }, take: 365 }
      },
    });
    if (!company) throw new Error('Company not found');
    ...
    const converted: any = {
     ...company,
      ...
      historicalPrices: company.historicalPrices.map(price => ({...price, volume: Number(price.volume) })),
    };
    return converted;
  }
